...
/* ----------------- SERVER CONFIG --------------------- */
/* Image */
#define BMP_INPUT_FILENAME "rb.bmp"
...
/* Encryption */
#define RSA_N           1591
#define RSA_PRIVATE_KEY 349 //Used only for the decrypt debug task
#define RSA_PUBLIC_KEY  13
...
QueueHandle_t xQueueEdges, xQueueImgEncrypted;
...
/* Image filename. Should be a 8bit bitmap image*/
const char* imageFilename = BMP_INPUT_FILENAME;


static void cannyTask()
{
	/** Initialization */
	struct ImgMessage stImgMessage;
	bitmap_info_header_t ih;

	while (1) {
		printf("Canny Task - Begin \n");
		time_t  beginMs = clock();

		/* Load image from file. This function allocates memory */
		const pixel_t* in_bitmap_data = load_bmp(imageFilename, &ih);
		if (in_bitmap_data == NULL) {
			fprintf(stderr, "main: BMP image not loaded.\n");
			return;
		}

		/* Find borders in image using canny. This function allocates memory */
		...
		/* Free memory allocated by load_bmp */
		free((pixel_t*)in_bitmap_data);

		/* Fill message struct */
		stImgMessage.pImage = out_bitmap_data;
		stImgMessage.height = ih.height;
		stImgMessage.width = ih.width;

		time_t endMs = clock();
		printf("Canny Task - End. Ellapsed ms: %d\n", endMs - beginMs);

		/* Send data to encrypt task */
		if (xQueueEdges != 0)
		{
			/* Block execution if the queue is full to avoid losing messages.
			This would be bad as the allocated memory wont be freed if the message is not sent*/
			xQueueSend(xQueueEdges, (void*)&stImgMessage, portMAX_DELAY);
			//printf("Sent\n");
		}

		/* Give a small delay for aperiodic/low priority tasks to have a chance to advance */
		vTaskDelay(1);
	}
}
/* Task to encrypt and compress the image sent from canny task */
static void encryptTask()
{
	...
	while (1) {
		if (xQueueEdges != 0)
		{
			/* Receive a message from queue. Block for some ticks if a message is not immediately available */
			if (xQueueReceive(xQueueEdges, &(stImgMessage), (TickType_t)20))
			{
				/* If we received a message, process it */
				//print it
				printf("Encrypt task - Queue message received \n");
				time_t  beginMs = clock();

				/* Check if pointer is not null */
				if (stImgMessage.pImage != 0) {

					/* Debug - Print some pixels before encryption*/
					//for (int i = 700; i < 900; i++) {
					//	printf("%d ", stImgMessage.pImage[i]);
					//}
					//printf("\n\n");

					/* Simple 1 to 1 encryption */
					//for (int i = 0; i < stImgMessage.height * stImgMessage.width; i++) {
					//	*(stImgMessage.pImage + i) = encryptInt(*(stImgMessage.pImage + i), 1591, 13);
					//}

					/* As after canny the image is binary, a group of pixels can be fitted into one*/
					int uiBitPosition = 0;
					int uiNewPixelIndex = 0; /*< To store the encoded pixels*/
					pixel_t usTempPixel = 0;
					/* For each pixel in the image */
					for (int i = 0; i < stImgMessage.height * stImgMessage.width; i++) {
						/* If the pixel is not 0 (Border) store as a bit in a temporal pixel variable */
						if (*(stImgMessage.pImage + i) != 0)
						{
							/* Bit operation to change only one bit*/
							usTempPixel |= (1 << uiBitPosition);
						}
						uiBitPosition++;
						/* After 8 bits store the temporal pixel variable */
						if (uiBitPosition == 8) {
							/* Encrypt the temporal pixel variable and store it in the image array (We can reuse it)*/
							stImgMessage.pImage[uiNewPixelIndex] = encryptInt(usTempPixel, RSA_N, RSA_PUBLIC_KEY);
							uiNewPixelIndex++;
							/* Reset variables*/
							uiBitPosition = 0;
							usTempPixel = 0;
					...
					/* Debug - Print some pixels after encryption*/
					//for (int i = 700; i < 900; i++) {
					//	printf("%d ", stImgMessage.pImage[i]);
					//}

					time_t endMs = clock();
					printf("Encrypt task - End. Ellapsed ms: %d\n", endMs - beginMs);
					...
					if (xQueueEdges != 0)
					{
						/* Block execution if the queue is full to avoid losing messages.
						This would be bad as the allocated memory wont be freed if the message is not sent*/
						xQueueSend(xQueueImgEncrypted, (void*)&stImgMessage, portMAX_DELAY);
					}
				}
			}
		}
...
static void decryptTask()
{
	...
	/*Load source image to get bitmap_info_header_t*/
	bitmap_info_header_t ih;
	const pixel_t* in_bitmap_data = load_bmp(imageFilename, &ih);
	if (in_bitmap_data == NULL) {
		fprintf(stderr, "main: BMP image not loaded.\n");
	...
	pixel_t* temp_bitmap_data = malloc(sizeof(pixel_t) * ih.width * ih.height);
	memset(temp_bitmap_data, 0, sizeof(pixel_t) * ih.width * ih.height);

	while (1) {
		if (xQueueImgEncrypted != 0)
		{
			if (xQueueReceive(xQueueImgEncrypted, &(stImgMessage), (TickType_t)20))
			{
				// if we received a message that means the data is updated
				//print it
				printf("Decrypt task - Queue message received \n");
				time_t  beginMs = clock();

				//Byte to Byte
				//for (int i = 0; i < stImgMessage.height * stImgMessage.width; i++) {
				//	*(stImgMessage.pImage + i) = encryptInt(*(stImgMessage.pImage + i), 1591, 349);
				//}
				...
				/* Decode and decrypt image */
				for (int i = 0; i <= (stImgMessage.height * stImgMessage.width) / 8; i++) {
					for (int j = 0; j < 8; j++) {
						if ((encryptInt(*(stImgMessage.pImage + i), RSA_N, RSA_PRIVATE_KEY) & (1 << j))) {
							*(temp_bitmap_data + i * 8 + j) = (pixel_t)255;
						}
					}
				}

				/* Save the image as a bmp*/
				if (save_bmp("out.bmp", &ih, stImgMessage.pImage)) {
					fprintf(stderr, "main: bmp image not saved.\n");
					return;
				}

				/* Free memory */
				free((pixel_t*)stImgMessage.pImage);

				time_t endMs = clock();
				printf("Decrypt and saved. Ellapsed ms: %d\n", endMs - beginMs);
			}
		}
	}
}
static void socketTask()
{
	...
	/* When the socket is connected start sending data*/
	size_t xLenToSend;
	printf("Socket Task - Ready to send\n");
	while (1) {
		BaseType_t xAlreadyTransmitted = 0, xBytesSent = 0;

		if (xQueueImgEncrypted != 0)
		{
			// receive a message on the created queue. Block for X ticks if a message is not immediately available
			if (xQueueReceive(xQueueImgEncrypted, &(stImgMessage), (TickType_t)20))
			{
				printf("Socket task - Queue message received \n");
				time_t  beginMs = clock();

				/* Serialize pixels. As they are short (2 bytes) they can not be sent directly */
				size_t encodedImageSize = (stImgMessage.width * stImgMessage.height) / 8; /*< divided by 8 as we encoded 8 binary pixel values in each pixel_t*/
				size_t xTotalLengthToSend = (encodedImageSize * 2) + 4; /*< x2 because short takes two bytes per pixel_t*/

				/* Allocate memory for the transmit buffer*/
				unsigned char* pcBufferToTransmit = (unsigned char*)malloc(xTotalLengthToSend + 4);
				int bufferIndex = 0;

				/* Set image width and height onto the message */
				pcBufferToTransmit[bufferIndex] = (unsigned char)(((unsigned short)stImgMessage.width >> 8) & 0xFF);
				bufferIndex++;
				pcBufferToTransmit[bufferIndex] = (unsigned char)((unsigned short)stImgMessage.width & 0xFF);
				bufferIndex++;

				pcBufferToTransmit[bufferIndex] = (unsigned char)(((unsigned short)stImgMessage.height >> 8) & 0xFF);
				bufferIndex++;
				pcBufferToTransmit[bufferIndex] = (unsigned char)((unsigned short)stImgMessage.height & 0xFF);
				bufferIndex++;

				/* Separate each pixel_t into two bytes. (16 bit -> 2 * 8 bit) */

				/* For each encoded byte */
				for (unsigned int n = 0; n < encodedImageSize; n++)
				{
					pcBufferToTransmit[bufferIndex] = (unsigned char)((stImgMessage.pImage[n] >> 8) & 0xFF);
					pcBufferToTransmit[bufferIndex + 1] = (unsigned char)(stImgMessage.pImage[n] & 0xFF);
					bufferIndex += 2; /*< Sizeof(pixel_t)*/
				}
				...
				if (conectedResult == 0)
				{
					...
					while (xAlreadyTransmitted < xTotalLengthToSend)
					{
						...
						if (xBytesSent >= 0)
						{
							...
							printf("Socket task - Sent. Ellapsed ms: %d\n", clock() - beginMs);
							/* Leave time for server to get all the bytes of one image.
							This way allows to simply count received bytes in the server to determine when an image is sent */
							vTaskDelay(2);
						}
						else
						{
				...
				/* Free the transmit buffer and the allocated memory in the cannyTask*/
				free(pcBufferToTransmit);
				free((pixel_t*)stImgMessage.pImage);
			}
		}
...
xTimerStart(xCheckTimer, 0);
	...
	***NOTE*** Tasks that use the network are created in the network event hook
	...
	/* Queue creation*/
	xQueueEdges = xQueueCreate(2, sizeof(struct ImgMessage));

	if (xQueueEdges == NULL)
	{
		perror("Queue for canny images processed not open");
		return 1;
	}

	xQueueImgEncrypted = xQueueCreate(2, sizeof(struct ImgMessage));

	if (xQueueEdges == NULL)
	{
		perror("Queue for encrypted images processed not open");
		return 1;
	}


	/* Task creation. Each step has higher priorty */
	TaskHandle_t cannyHandle;
	xTaskCreate(cannyTask, (signed char*)"CannyTask", 1000, NULL, 3, &cannyHandle);

	TaskHandle_t encryptHandle;
	xTaskCreate(encryptTask, (signed char*)"Encrypt", 1000, NULL, 4, &encryptHandle);

	//xTaskHandle decryptHandle;
	//xTaskCreate((pdTASK_CODE)decryptTask, (signed char *)"Decrypt", 1000, NULL, 5, &decryptHandle);

	TaskHandle_t socketHandle;
	xTaskCreate(socketTask, (signed char*)"Socket", 1000, NULL, 6, &socketHandle);
